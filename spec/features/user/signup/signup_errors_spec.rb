...
feature "Inscription d'un user" do
  ...
  scenario "Inscription avec toutes les erreurs possibles" do

    test 'Un user s’inscrit en faisant toutes les erreurs possibles'
    ...
    # Le candidat rejoint le site
    visit_home
    Benoit clique le link 'Poser sa candidature'
    La feuille a pour titre 'Candidature Icare'

    def get_captcha_value
      # On récupère les valeurs du captcha
      x = page.execute_script("return $('span#xcap').html()").to_i
      y = page.execute_script("return $('span#ycap').html()").to_i
      captcha_value = (x + y).to_s
    end

    def benoit_soumet_le_form
      Benoit clique le bouton 'Enregistrer et poursuivre l’inscription', dans: 'form#form_user_signup'
      La feuille a pour titre 'Candidature Icare' # pour attendre
    end

    form_id = 'form_user_signup'
    form_jid = "form##{form_id}"

    test 'Benoit soumet le formulaire d’identité en mettant de mauvaises valeurs'
    benoit_soumet_le_form

    # Erreur
    La feuille contient le message erreur 'Il faut fournir le pseudo'

    Benoit remplit le champ 'user_pseudo', avec: data_signup[:pseudo], dans: form_jid
    benoit_soumet_le_form
    La feuille contient le message erreur 'Il faut fournir votre mail'

    Benoit remplit le champ 'user_mail', avec: data_signup[:mail], dans: form_jid
    benoit_soumet_le_form
    ...
    Benoit remplit le champ 'user_password', avec: data_signup[:password], dans: form_jid
    benoit_soumet_le_form
    La feuille contient le message erreur 'La confirmation du mot de passe ne correspond pas'

    Benoit remplit le champ 'user_password_confirmation', avec: 'nimmporte quoi', dans: form_jid
    benoit_soumet_le_form
    La feuille contient le message erreur 'La confirmation du mot de passe ne correspond pas'

    Benoit remplit le champ 'user_password_confirmation', avec: data_signup[:password], dans: form_jid
    benoit_soumet_le_form
    La feuille contient le message erreur 'Il faut fournir le captcha pour nous assurer que vous n’êtes pas un robot'

    captcha_value = get_captcha_value.freeze

    bad_captcha = captcha_value == '10' ? '12' : '10'
    Benoit remplit le champ 'user_captcha', avec: bad_captcha, dans: form_jid
    benoit_soumet_le_form
    La feuille contient le message erreur 'Le captcha est mauvais, seriez-vous un robot'
  ...
  # Inscription avec des erreurs tout du long
