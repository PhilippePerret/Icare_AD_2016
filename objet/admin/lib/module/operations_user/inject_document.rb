...
class << self
  ...
  def exec_inject_document
    ...
    user.icmodule   || raise('L’icarien choisi ne possède pas de module courant. Impossible de lui injecter un document d’étape.')
    user.icetape    || raise('L’icarien choisi ne possède pas d’étape courant. Impossible de lui injecter un document.')
    absmodule = user.icmodule.abs_module
    icetape   = user.icetape
    ...
    flash "Documnent “#{nom_doc}” (##{new_doc_id}) injecté dans l'étape #{absetape.numero}-#{absetape.titre} du module #{absmodule.name} de #{user.pseudo}."+
